...
@include('layouts/navbar/navbar_admin')
            <div class="content p-4">
                <h1 class="text-center">Form Tambah Mapel</h1>      
            
                <form action="{{ url('mapel/add') }}" method="POST" id="formTambahMapel">
                    {{ csrf_field() }}
                    <div class="form-group">
                        ...
                        <div class="input-group" id="nama_mapel_nest">
                            ...
                            <input type="text" class="form-control" id="nama_mapel" name="nama_mapel" placeholder="Masukkan nama mapel" value="{{ old('nama_mapel') }}">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="bidang_keahlian" id="bidang_keahlian" class="form-control">
                            @foreach($bidangKeahlian as $bk)
                                <option value="{{ $bk->id_daftar_bidang }}" @if(old('bidang_keahlian') == $bk->id_daftar_bidang) selected @endif>{{ $bk->bidang_keahlian }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="btn-group btn-block">
                        ...
                        <button type="submit" class="btn btn-membesar btn-primary" data-toggle="tooltip" data-placement="bottom" title="Tambah Data Mapel"><i class="fas fa-plus"></i></button>
                    </div>
                </form>
            </div>
        </div>
