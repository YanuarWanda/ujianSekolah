...
@include('layouts/navbar/navbar_admin')
            <div class="content p-4">
                <h1 class="text-center">Form Edit Kelas</h1>

                <form action="{{ url('kelas/edit', base64_encode($kelas->id_kelas)) }}" method="POST" id="formEditKelas">
                    {{ csrf_field() }}
                    <div class="form-group">
                        <label for="nama_kelas" class="col-form-label"></label>
                        <div class="input-group" id="nama_kelas_nest">
                            <div class="input-group-prepend">
                                <div class="input-group-text"><i class="fas fa-id-badge"></i></div>
                            </div>
                            <input type="text" class="form-control" id="nama_kelas" name="nama_kelas" placeholder="Masukkan nama kelas" value="{{ $kelas->nama_kelas }}">
                        </div>
                    </div>
                            ...
                            <div class="col-md-9">
                                <select name="jurusan" id="jurusan" class="form-control">
                                    @foreach($jurusan as $j)
                                        <option value="{{ $j->id_jurusan }}" @if($kelas->id_jurusan == $j->id_jurusan) selected @endif>{{ $j->nama_jurusan }}</option>
                                    @endforeach     
                                </select>
                            </div>
                            <div class="col-md-3">
                    ...
                    <div class="btn-group btn-block">
                        ...
                        <button type="submit" class="btn btn-membesar btn-primary" data-toggle="tooltip" data-placement="bottom" title="Update Data Kelas"><i class="fas fa-edit"></i></button>
                    </div>
                </form>      
            </div>
        </div>
