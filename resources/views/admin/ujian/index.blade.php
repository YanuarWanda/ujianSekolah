                        ...
                        <div class="col-lg-6">
                            <div class="card card-ujian">
                                <div class="card-header text-center">
                                    <a href="{{ url('ujian/edit', base64_encode($u->id_ujian)) }}" data-toggle="tooltip" data-placement="top" title="Detail Ujian" class="title">{{ $u->judul_ujian }}</a> <br> <b><i>{{ $u->mapel->nama_mapel }}</i></b>
                                    <div class="close-btn bisaHover">
                                        <span href="{{ url('ujian/delete', base64_encode($u->id_ujian)) }}" class="deleteButton"><i class="fas fa-times fa-1x text-danger"></i></span>
                                    </div>
                                </div>
                                <div class="card-body">
                                    <div class="row pb-0">
                                    ...
                                    <hr>
                                    @if($u->status == 'posted')
                                        <h4><span class="btn btn-success unpostModal" href="{{ url('/ujian/draft', base64_encode($u->id_ujian)) }}">Posted</span> | <small>sampai {{ $u->tanggal_kadaluarsa }}</small></h4>
                                    @else
                                    ...
                                    <div class="row">
                                        <div class="col-4">
                                            <a href="{{ url('ujian/nilai', base64_encode($u->id_ujian)) }}" class="btn btn-primary btn-block"><i class="fas fa-book fa-3x"></i><br><span class="text-center"><i>Daftar Nilai</i></span></a>
                                        </div>
                                        <div class="col-8">
                                            ...
                                            <div class="btn-group btn-block menuAddRemed_{{$us}}" aria-expanded="false" style="display:none">
                                                @if($jumlahUjianRemed == 2)
                                                    ...
                                                    <a class="btn btn-primary btn-membesar half" href="{{ url('ujian/remed/add', base64_encode($u->id_ujian)) }}">Remed 3</a>
                                                @elseif($jumlahUjianRemed == 1)
                                                    <a class="btn btn-secondary btn-membesar half" disabled="">Remed 1</a>
                                                    <a class="btn btn-primary btn-membesar half" href="{{ url('ujian/remed/add', base64_encode($u->id_ujian)) }}">Remed 2</a>
                                                    <a class="btn btn-secondary btn-membesar half" disabled="">Remed 3</a>
                                                @elseif($jumlahUjianRemed == 0)
                                                    <a class="btn btn-primary btn-membesar half" href="{{ url('ujian/remed/add', base64_encode($u->id_ujian)) }}">Remed 1</a>
                                                    <a class="btn btn-secondary btn-membesar half" disabled="">Remed 2</a>
                                                    <a class="btn btn-secondary btn-membesar half" disabled="">Remed 3</a>
                                                @endif
                                            </div>
                                            ...
                                            <div class="btn-group btn-block menuDetailRemed_{{$us}}" aria-expanded="false" style="display:none">
                                                @foreach($sRemed as $sr => $isiSR)    
                                                    @if($u->id_ujian == $isiSR->id_ujian)
                                                        @if($isiSR->remed_ke == 1)
                                                            <a href="{{ url('ujian/remed/detail', base64_encode($isiSR->id_ujian_remedial)) }}" class="btn btn-success btn-membesar half">Remed 1</a>
                                                            <?php break; ?>
                                                        @elseif($sr == count($sRemed)-1)
                                                            <a class="btn btn-secondary btn-membesar half" disabled="">Remed 1</a>
                                                        @endif
                                                ...
                                                @foreach($sRemed as $sr => $isiSR)    
                                                    @if($u->id_ujian == $isiSR->id_ujian)
                                                        @if($isiSR->remed_ke == 2)
                                                            <a href="{{ url('ujian/remed/detail', base64_encode($isiSR->id_ujian_remedial)) }}" class="btn btn-success btn-membesar half">Remed 2</a>
                                                            <?php break; ?>
                                                        @elseif($sr == count($sRemed)-1)
                                                            <a class="btn btn-secondary btn-membesar half" disabled="">Remed 2</a>
                                                        @endif
                                                ...
                                                @foreach($sRemed as $sr => $isiSR)    
                                                    @if($u->id_ujian == $isiSR->id_ujian)
                                                        @if($isiSR->remed_ke == 3)
                                                            <a href="{{ url('ujian/remed/detail', base64_encode($isiSR->id_ujian_remedial)) }}" class="btn btn-success btn-membesar half">Remed 3</a>
                                                            <?php break; ?>
                                                        @elseif($sr == count($sRemed)-1)
                                                            <a class="btn btn-secondary btn-membesar half" disabled="">Remed 3</a>
                                                        @endif
                                ...
                                <div class="card-footer">
                                    <small class="text-muted">Di buat oleh @if($u->nama) {{ $u->nama }} @else Admin @endif pada {{ $u->tanggal_post }}</small>
                                </div>
                            </div>
                        </div>
                    @endforeach
                ...
                <div class="row">
                    <div class="col-3">
                        <div class="btn-float-add-ujian">
                            <a href="{{ url('ujian/add') }}" class="btn btn-primary"><i class="fas fa-plus">&nbsp;Tambah Ujian</i></a>
                        </div>
                    </div>
                    <div class="col-9">
        ...
        <div class="modal-content">
            <form id="post-data_{{$us}}" method="POST" action="{{url('/ujian/post', base64_encode($u->id_ujian))}}">
                ...
                <div class="modal-body">
                    ...
                    <select name="kelas[]" id="kelas_{{$us}}" class="form-control selectpicker show-tick" data-live-search="true" multiple data-selected-text-format="values" data-size="5" data-style="form-control" multiple>
                        @foreach($kelas as $k)
                            <option>
                                {{ $k->nama_kelas }}
                            </option>
