                        ...
                        <div>
                            @if($sValue->bankSoal->tipe != 'MC')
                                @foreach($pilihanAsli as $p)
                                    <div class="radio">
                                        <label class="radio-inline"><input type="radio" name="jawaban_{{$sValue->id_bank_soal}}" class="jawaban_{{$sValue->id_bank_soal}} Jawab" value="{{ $p }}" onclick="simpanJawaban('jawaban_'+{{ Auth::user()->id_users }}+'_'+{{ $sValue->id_ujian }}+'_'+{{ $sValue->id_bank_soal }}, '{{ $p }}' )" autocomplete="off" data-pindah="{{ $sValue->id_bank_soal }}">{!! $p !!}</label>
                                    </div>
                                @endforeach
                            @else
                                @foreach($pilihanAsli as $p)
                                    <div class="checkbox">
                                        <label><input type="checkbox" name="jawaban_{{$sValue->id_bank_soal}}[]" class="jawaban_{{$sValue->id_bank_soal}} Jawab" value="{{ $p }}" onclick="simpanJawabanMC('jawaban_'+{{ Auth::user()->id_users }}+'_'+{{ $sValue->id_ujian }}+'_'+{{ $sValue->id_bank_soal }}, '{{ $p }}' )" autocomplete="off" data-pindah="{{ $sValue->id_bank_soal }}">{!! $p !!}</label>
                                    </div>
                                @endforeach
                            @endif
                        </div>
...
<script>
    $('.Jawab').on('click', function(){
        ...
        simpanJawaban('pindah_'+$indikator+'_'+$idUser+'_'+$idUjian, 'pindah_'+$indikator);
    }); 
